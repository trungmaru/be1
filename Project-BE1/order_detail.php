...
<main>
  ...
  <div class="container">
    ...
    <form action="order_detail.php?id=<?php echo $id; ?>" method="POST">
      <select id="status" name="status" class="d-inline-block form-select" style="width: 20%;">
        <optgroup class="form-role">
          <option value="0" <?php if ($orderInfo['status'] == 0) {
                              echo 'selected';
                            } ?>>Chờ xác nhận</option>
          <option value="1" <?php if ($orderInfo['status'] == 1) {
                              echo 'selected';
                            } ?>>Đã xác nhận</option>
          <option value="2" <?php if ($orderInfo['status'] == 2) {
                              echo 'selected';
                            } ?>>Đã hủy</option>
        </optgroup>
      </select>
